...
const Users = () => {
  ...
  const [formData, setFormData] = useState({ username: "", full_name: "", email: "", role: "", password: "" });
  ...
  return (
    <div className="users-container">
      ...
      <div className="search-box">
        ...
        <input type="text" placeholder="Buscar por nombre, usuario, email o rol" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </div>
          ...
          <tbody>
            {filteredUsers.map(u => (
              <tr key={u.id}>
                ...
                <td><span className="role-badge" style={getRoleStyle(u.role)}><Shield size={14} style={{ marginRight: 4 }} /> {roles.find(r => r.value === u.role)?.label}</span></td>
                <td>
                  <button className={`status-btn ${u.is_active ? "active" : "inactive"}`} onClick={() => toggleStatus(u.id)}>
                    {u.is_active ? "Activo" : "Inactivo"}
                  </button>
                </td>
                <td>
                  <button className="action-btn edit" onClick={() => handleEdit(u)}><Edit size={16} /></button>
                  <button className="action-btn delete" onClick={() => handleDeleteClick(u)}><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className={`modal ${isClosing ? "closing" : ""}`}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group password-field">
                ...
                <input type={showPassword ? "text" : "password"} value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} required={!editingUser} minLength="6" />
      ...
      {showConfirmModal && (
        <div className={`modal-overlay ${isClosingConfirm ? "closing" : confirmClass}`}>
          <div className={`modal ${isClosingConfirm ? "closing" : confirmClass}`}>
            ...
            <div className="modal-actions" style={{ justifyContent: "center", gap: "1rem" }}>
              <button className="confirm-btn" onClick={confirmDelete}>Sí, Eliminar</button>
